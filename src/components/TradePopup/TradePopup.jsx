...
if (userWinLose === "Off") {
  ...
  newBalance = numericBalance + numericEstimatedIncome - 0.5;
        ...
        setBalance(newBalance); // Update local state balance
  ...
  return isOpen ? (
    <div className="fixed inset-0 flex items-end justify-center z-50 bg-black bg-opacity-70 trade-popup-main-cont">
      <div className="bg-gray-800 p-6 rounded-t-lg h-[98%] flex flex-col">
        <h1 style={{ fontSize: "22px", textAlign: "start", color: "gray" }}>
          {cryptoData.name}{" "}
        </h1>
        <h2
        ...
        <input
          ...
          className="mt-2 w-full p-3 rounded border border-gray-600"
          required
        />

        <div className="mt-4 gap-2 mb-2">
          <h3 className="text-green-400 mt-4 mb-4 select-expiration-time-txt">
            Select expiration time
          </h3>
          {expirationOptions.map((option, index) => (
            <button
              key={index}
              className={`px-4 py-2 mr-2 rounded mb-2 expiration-times ${
        ...
        {selectedExpiration && (
          <div className="mt-4 bg-gray-700 min-max-container">
            <div className="minimum-trading-amount-container">
              <span>Minimum Amount</span>
              <p> ${selectedExpiration.minTradeAmount}</p>
            </div>
            <div className="maximum-trading-amount-container">
              <span>Maximum Amount</span>
              <p>${selectedExpiration.maxTradeAmount}</p>
            </div>
          </div>
        )}
        ...
        <p className="mt-4 estimated-income">
